...
if npks > 1 
    ...
    %changed by CLin to consider consecutive effect in neighbours
    tmp=0.*im;
   % tmp_temp=tmp;
    for i=1:nmx
    ...
    for i=1:nmx
        roi=tmp(max(1,(mx(i,1)-floor(sz/2))):min(size(im,1),(mx(i,1)+(floor(sz/2)+1))),max(1,(mx(i,2)-floor(sz/2))):min((mx(i,2)+(floor(sz/2)+1)),size(im,2))) ;
        %roi=tmp_temp(max(1,(mx(i,1)-floor(sz/2))):min(size(im,1),(mx(i,1)+(floor(sz/2)+1))),max(1,(mx(i,2)-floor(sz/2))):min((mx(i,2)+(floor(sz/2)+1)),size(im,2))) ;
        [mv,indi]=max(roi);
        [mv,indj]=max(mv);
        tmp( max(1,(mx(i,1)-floor(sz/2))):min(size(im,1),(mx(i,1)+(floor(sz/2)+1))),max(1,(mx(i,2)-floor(sz/2))):min(size(im,2),(mx(i,2)+(floor(sz/2)+1))))=0;
       %[max(1,mx(i,1)-floor(sz/2))+indi(indj)-1,min(size(im,2),mx(i,2)-floor(sz/2))+indj-1]
